"use client";

import React from "react";
import Link from "next/link";
import Image from "next/image";

import serviceImg2 from "../../../../public/images/services/services2.jpg";

const ExteriorTabContent: React.FC = () => {
  return (
    <>
      <div className="tab-content">
        <div 
          className="services-tab-content"
          data-aos="fade-up"
          data-aos-delay="100"
          data-aos-duration="600"
          data-aos-once="false"
        >
          <Image src={serviceImg2} alt="image" width={850} height={270} />
          <p>
            We are specialized in the privates houses creation. Our architecture
            is an innovative and creative retranscription of the lifestyle and
            codes of the Arcachon basin. We design houses that are integrated
            into their environment. We imagine for you a haven of peace.
          </p>
          <Link href="/portfolio-2/">VIEW ALL PRODUCTS</Link>
        </div>
      </div>
    </>
  );
};

export default ExteriorTabContent;
